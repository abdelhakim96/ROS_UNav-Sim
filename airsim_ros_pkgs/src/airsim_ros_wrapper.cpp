...
void AirsimROSWrapper::initialize_airsim()
{
    ...
    try {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            airsim_client_ = std::unique_ptr<msr::airlib::RpcLibClientBase>(new msr::airlib::RovRpcLibClient(host_ip_));
        }
        else {
...
void AirsimROSWrapper::initialize_ros()
{
    ...
    pwm_cmd_duration_ = 0.05;
    //current_dist_duration_ = 0.05;
     // todo rosparam
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            drone->pwm_cmd_sub = nh_private_.subscribe<airsim_ros_pkgs::PwmCmd>(curr_vehicle_name + "/pwm_cmd", 1, boost::bind(&AirsimROSWrapper::pwm_cmd_cb, this, _1, vehicle_ros->vehicle_name));

            drone->current_dist_sub = nh_private_.subscribe<airsim_ros_pkgs::CurrentDist>(curr_vehicle_name + "/current_dist", 1, boost::bind(&AirsimROSWrapper::current_dist_cb, this, _1, vehicle_ros->vehicle_name));
        ...
        for (auto& curr_sensor_map : vehicle_setting->sensors) {
            ...
            if (sensor_setting->enabled) {
                ...
                case SensorBase::SensorType::Airspeed: {
                    ...
                    sensor_publisher.publisher = nh_private_.advertise<airsim_ros_pkgs::Altimeter>(curr_vehicle_name + "/airspeed/" + sensor_name, 10);
                    break;
                }
...
bool AirsimROSWrapper::takeoff_srv_cb(airsim_ros_pkgs::Takeoff::Request& request, airsim_ros_pkgs::Takeoff::Response& response, const std::string& vehicle_name)
{
    ...
    if (request.waitOnLastTask)
        static_cast<msr::airlib::RovRpcLibClient*>(airsim_client_.get())->takeoffAsync(20, vehicle_name)->waitOnLastTask(); // todo value for timeout_sec?
    // response.success =
    else
        static_cast<msr::airlib::RovRpcLibClient*>(airsim_client_.get())->takeoffAsync(20, vehicle_name);
    // response.success =
...
bool AirsimROSWrapper::takeoff_all_srv_cb(airsim_ros_pkgs::Takeoff::Request& request, airsim_ros_pkgs::Takeoff::Response& response)
{
    ...
    if (request.waitOnLastTask)
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            static_cast<msr::airlib::RovRpcLibClient*>(airsim_client_.get())->takeoffAsync(20, vehicle_name_ptr_pair.first)->waitOnLastTask(); // todo value for timeout_sec?
    // response.success =
    else
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            static_cast<msr::airlib::RovRpcLibClient*>(airsim_client_.get())->takeoffAsync(20, vehicle_name_ptr_pair.first);
    // response.success =
...
bool AirsimROSWrapper::land_srv_cb(airsim_ros_pkgs::Land::Request& request, airsim_ros_pkgs::Land::Response& response, const std::string& vehicle_name)
{
    ...
    if (request.waitOnLastTask)
        static_cast<msr::airlib::RovRpcLibClient*>(airsim_client_.get())->landAsync(60, vehicle_name)->waitOnLastTask();
    else
        static_cast<msr::airlib::RovRpcLibClient*>(airsim_client_.get())->landAsync(60, vehicle_name);
...
bool AirsimROSWrapper::land_all_srv_cb(airsim_ros_pkgs::Land::Request& request, airsim_ros_pkgs::Land::Response& response)
{
    ...
    if (request.waitOnLastTask)
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            static_cast<msr::airlib::RovRpcLibClient*>(airsim_client_.get())->landAsync(60, vehicle_name_ptr_pair.first)->waitOnLastTask();
    else
        for (const auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
            static_cast<msr::airlib::RovRpcLibClient*>(airsim_client_.get())->landAsync(60, vehicle_name_ptr_pair.first);
...
nav_msgs::Odometry AirsimROSWrapper::get_odom_msg_from_rov_state(const msr::airlib::RovState& drone_state) const
{
    nav_msgs::Odometry odom_msg;
...
ros::Time AirsimROSWrapper::update_state()
{
    ...
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            drone->curr_drone_state = rpc->getRovState(vehicle_ros->vehicle_name);
            ...
            vehicle_ros->curr_odom = get_odom_msg_from_rov_state(drone->curr_drone_state);
        }
        else {
            auto car = static_cast<CarROS*>(vehicle_ros.get());
    ...
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            if (drone->has_vel_cmd) {
                std::lock_guard<std::mutex> guard(drone_control_mutex_);
                static_cast<msr::airlib::RovRpcLibClient*>(airsim_client_.get())->moveByVelocityAsync(drone->vel_cmd.x, drone->vel_cmd.y, drone->vel_cmd.z, vel_cmd_duration_, msr::airlib::DrivetrainType::MaxDegreeOfFreedom, drone->vel_cmd.yaw_mode, drone->vehicle_name);
            }
